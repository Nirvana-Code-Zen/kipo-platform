...
import type { StampsProgressProps } from "./types"

export function StampsProgress({ stamps, onBuyClick }: StampsProgressProps) {
      ...
      <div className="flex flex-col items-center">
        <div className="relative w-40 h-40 mb-4">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-muted-foreground mt-1">Timbradas</span>
          </div>
        </div>
        <div className="flex flex-wrap justify-center gap-3 text-xs">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-primary flex-shrink-0" />
            <span className="text-muted-foreground whitespace-nowrap">Timbradas</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        {onBuyClick && (
          <button
            onClick={onBuyClick}
            className="mt-4 text-xs font-medium transition-opacity hover:opacity-80 text-[var(--brand)]"
          >
            Comprar timbres
          </button>
        )}
