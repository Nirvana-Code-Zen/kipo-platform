...
import { REGISTER_ERROR_MESSAGE } from './constants'
  ...
  return (
    <>
      ...
      {error && (
        ...
        >
          {REGISTER_ERROR_MESSAGE[error.kind] ?? 'Error desconocido'}
        </div>
      )}
      ...
      {tab === 'phone' && isOtpPending && (
        <form onSubmit={handleVerifyOtp} className='flex flex-col gap-3.5'>
          <p className='text-sm text-text-body font-sans leading-normal m-0'>
            Código enviado a <strong className='text-foreground'>{pendingOtp?.phone}</strong> vía SMS
          </p>
