...
export const LoginView = () => {
  ...
  const handleSocial = async (provider: 'google' | 'apple' | 'facebook') => {
    // Social auth: backend returns the OAuth redirect URL.
    // For now this is a placeholder — wire to your OAuth flow.
        ...
        {tab === 'phone' && isOtpPending && (
          <form onSubmit={handleVerifyOtp} className='space-y-4'>
            <p className='text-sm text-gray-600'>
              Código enviado a <strong>{pendingOtp?.phone}</strong> vía {pendingOtp?.channel === 'whatsapp' ? 'WhatsApp' : 'SMS'}
            </p>
            <div>
