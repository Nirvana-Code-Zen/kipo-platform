...
export function CreateInvoiceSheet({ isOpen, onClose, onSubmit }: CreateInvoiceSheetProps) {
      ...
      <div className="relative z-10 w-full sm:max-w-2xl bg-card rounded-t-2xl sm:rounded-2xl shadow-2xl animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300 flex flex-col max-h-[92dvh]">
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-border flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">El borrador se puede timbrar después</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
          <CreateInvoiceForm
            onSubmit={(invoice) => { onSubmit(invoice); onClose() }}
            onCancel={onClose}
          />
        </div>
      </div>
